...
public class NetherManager
{
    ...
    {
        /*
         * BlockVector location = nether.getInternalArea().getCenter(); Chunk
         * chunk = world.getChunkAt(location.getBlockX(), location.getBlockZ());
         * NetherList list = netherMap.get(getChunkId(chunk)); if (list == null)
         * { list = new NetherList(); netherMap.put(getChunkId(chunk), list); }
         * list.add(nether);
         */
    }
    ...
    {
        // Make sure the current world is loaded
        ...
        if (debugLogging)
        {
            String formatMessage = "NG: Buiding temporary portal for %s at (%d, %d, %d)";
            String message = String.format(formatMessage, player.getName(), centerBlock.getX(), centerBlock.getY(), centerBlock.getZ());
            log.info(message);
        }
    ...
    protected Location findPlaceToStand(Location startLocation)
    {
        ...
        {
            if (isOkToStandOn(mats[0]) && isOkToStandIn(mats[1]) && isOkToStandIn(mats[2]))
            {
    ...
    protected void finishTeleport(NetherPlayer playerData, World world)
    {
        ...
        if (player != null)
        {
            ...
            Location location = new Location(world, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), currentLocation.getYaw(), currentLocation.getPitch());
            ...
            {
                // Go one up- findPlaceToStand returns a block, not a place to
                // tp to.
            ...
            if (debugLogging)
            {
                ...
                String message = String.format(formatMessage, player.getName(), currentLocation.getBlockX(), currentLocation.getBlockY(), currentLocation.getBlockZ(), targetLocation.getBlockX(), targetLocation.getBlockY(), targetLocation.getBlockZ());
                log.info(message);
            }
            ...
            if (sourcePortal != null && sourcePortal.getType().isTracked() && worldData != null)
            {
                ...
                // Auto-bind if not bound, and you have permission to create at
                // least the portal
    ...
    {
        /*
         * Chunk chunk = world.getChunkAt(position.getBlockX(),
         * position.getBlockZ()); NetherList list =
         * netherMap.get(getChunkId(chunk)); if (list == null) return null;
         * 
         * for (PortalArea nether : list) { if
         * (nether.getExternalArea().contains(position)) { return nether; } }
         */
        return null;
    }
    ...
    public NetherWorld getNextWorld(World currentWorld)
    {
        ...
        if (targetWorld == null)
        {
            ...
            // TODO - Persistence: a way to get entity could without getting a
            // whole list!
            if (allWorlds.size() == 1)
            {
    ...
    public void onPlayerDeath(Player player, EntityDeathEvent event)
    {
        ...
        if (homeWorld != null)
        {
            ...
            if (netherHome != null)
            {
                ...
                Location spawnPoint = new Location(home, spawn.getX(), spawn.getY(), spawn.getZ(), player.getLocation().getYaw(), player.getLocation().getPitch());
    ...
    public boolean requestBlockList(World currentWorld, String worldName, BlockVector center, int radius, BlockRequestListener listener)
    {
        ...
        /*
         * if (world.isChunkLoaded(chunk)) { request.dispatch(); } else {
         */
